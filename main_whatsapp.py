...
def process_whatsapp_message():
    ...
    try:
        ...
        if not incoming_message:
            ...
            response.message(
                "Silakan kirim pesan untuk mendapatkan respons dari AI Assistant."
            )
            return str(response)
        ...
        print(f"Sent response to {user_id}: {ai_response[:100]}...")
        ...
        response = MessagingResponse()
        response.message(
            "Maaf, saya mengalami kesalahan dalam memproses permintaan Anda. Silakan coba lagi."
        )
        return str(response)
...
def whatsapp_twilio_webhook():
    ...
    Receives incoming messages and returns AI responses.
    """
...
def health_check():
    ...
    return {
        "status": "healthy",
        "message": "RAG WhatsApp bot is running",
        "language": "Indonesian",
    }
...
def admin_dashboard():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            db.session.commit()
            print(
                f"File uploaded successfully with chunk_size={chunk_size}, chunk_overlap={chunk_overlap}"
            )
            files = get_file_status()
            return render_template(
                "admin_dashboard.html",
...
def embed_files():
    ...
    files = get_file_status()
    return render_template(
        "admin_dashboard.html", user=current_user, files=files, message=msg
    )
...
def preview_chunking():
    ...
    if file:
        print(
            f"[PREVIEW] File received: {file.filename}, size: {len(file.read())} bytes"
        )
    ...
    print(
        f"[PREVIEW] Received file: {file.filename if file else None}, chunk_size={chunk_size}, chunk_overlap={chunk_overlap}"
    )
    ...
    try:
        if filetype == "pdf":
            ...
            try:
                ...
                for i, d in enumerate(docs[:10]):
                    ...
                    if len(preview_docs) >= 3:
                        break
                print(
                    f"[PREVIEW] Using {len(preview_docs)} non-empty pages for preview"
                )
            ...
            try:
                ...
                df = pd.read_csv(file, nrows=100)  # type: ignore
                ...
                print(
                    f"[PREVIEW] CSV: loaded {len(df)} rows, preview_docs={len(preview_docs)}"
                )
            except Exception as e:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Failed to process the CSV file. It may be corrupt or unreadable.",
                        }
                    ),
                    400,
                )
            ...
            try:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Failed to process the TXT file. It may be corrupt or unreadable.",
                        }
                    ),
                    400,
                )
        ...
        if not preview_docs:
            print(
                "[PREVIEW] No extractable text found in the first 10 pages of the PDF."
            )
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "No extractable text found in the first 10 pages of the PDF. The file may be scanned images or empty.",
                    }
                ),
                200,
            )
        ...
        print(
            f"[PREVIEW] About to call split_documents_by_type with {len(preview_docs)} docs"
        )
        chunks = split_documents_by_type(
            preview_docs, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )
        ...
        if not chunks:
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "No chunks could be generated from the file preview. The file may be empty or not contain extractable text.",
                    }
                ),
                200,
            )
        ...
        for i, chunk in enumerate(chunks[:10]):
            preview_chunks.append(
                {"chunk_number": i + 1, "content": chunk.page_content}
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": f"An unexpected error occurred during preview: {str(e)}",
                }
            ),
            500,
        )
...
def send_whatsapp_reply(chat_id, message_text):
    ...
    try:
        ...
        print(f"[WAHA] Reply sent successfully. Response: {response.text}")
    except requests.exceptions.HTTPError as err:
        # This will catch the 404 error and others
        print(f"[WAHA] Error sending reply: {err}")
    except Exception as e:
        # This will catch other errors like connection timeouts
        print(f"[WAHA] An unexpected error occurred: {e}")

@app.route("/api/whatsapp/webhook", methods=["POST"])
def whatsapp_webhook():
    try:
        # Force parsing the request data as JSON
        payload = request.get_json(force=True)
        print(f"🔥 [WAHA DEBUG] Incoming JSON: {payload}")

        # 1. Explicitly ignore any event that is not a 'message'
        if payload.get("event") != "message":
            print(f"Ignoring event: {payload.get('event')}")
            return jsonify({"status": "ignored, not a message event"}), 200

        # 2. Extract data from the correct location ('payload' object)
        message_payload = payload.get("payload", {})
        user_message = message_payload.get("body")
        ...
        if user_message and chat_id:
            ...
            print(f"✅ Balasan terkirim ke {chat_id}")

            return jsonify({"status": "ok"}), 200
        else:
            # This will now only trigger if the 'message' event is missing 'body' or 'from'
            print(f"❌ [WAHA DEBUG] Missing 'body' or 'from' in message payload. Payload: {message_payload}")
            return jsonify({"error": "Missing 'body' or 'from' in message payload"}), 400
...
@app.route("/admin/delete_vector_db", methods=["POST"])
@login_required
